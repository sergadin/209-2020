			...
			case 's':
				{
					...
					else if(strstr(com,"s_del")==com)
					{
						...
						scanf("%s %lf %d", name, &rt, &gr);
						student * prime = new student( name, rt, gr);
						int ret = sess->del(prime);
						if(ret == 1)	printf("Not in sess!\n");
						else
						{
							printf("del_ited - ");
							prime->print();
...
void Data_base :: do_select(comand * cmd, Session * sess)
{
	if(cmd->c_gr != COND_NONE)
	{
		...
		if( G == EQ )
		{
			hash_node * curr = my_hash->root[index];
			while(curr)
			{
				if(curr->group == gr)
				{
					if(cmd->c_name != COND_NONE)	curr->my_tree->do_select(cmd, sess);
					//else	curr->my_list->do_select(cmd, sess);
					break;
				}
				curr = curr->next;
			}
