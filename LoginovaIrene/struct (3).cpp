...
void hash :: do_select( comand * cmd, Session * sess)
{
	hash_node * curr;
	cond_type G = cmd->c_gr;
	int gr = cmd->get_gr();
	int index = hash_fun(cmd);
	int i;
...
int Session :: inside(student * elem)
{
	...
	for(int i = 0; i < quan_st; i++)
	{
		if(strcmp(data[i]->get_name(), my_name)!=0)
			continue;
		if(data[i]->get_gr() != gr)
			continue;
		if(eq(data[i]->get_rt() - rt)!=1)
			continue;
		return 1;
	}
	return 0;
