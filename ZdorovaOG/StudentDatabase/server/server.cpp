...
void Server::Close() {
  ...
  for (auto &&clientSocket : _clientSockets) clientSocket->Close();
...
void Server::Listen() {
  ...
  if (_socket == -1) throw std::runtime_error(std::string(strerror(errno)));

  {
    int option = 1;

    setsockopt(_socket, SOL_SOCKET, SO_REUSEADDR,
               reinterpret_cast<const void *>(&option), sizeof(option));
  }
  ...
  {
    int result =
        bind(_socket, reinterpret_cast<const sockaddr *>(&socketAddress),
  ...
  while (1) {
    ...
    int numFileDescriptors = select(highestFileDescriptor + 1, &clientSocketSet,
                                    nullptr,   // no descriptors to write into
                                    nullptr,   // no descriptors with exceptions
                                    nullptr);  // no timeout

    if (numFileDescriptors == -1) break;
    ...
    for (int i = 0; i <= highestFileDescriptor; i++) {
      if (!FD_ISSET(i, &clientSocketSet)) continue;
      ...
      if (i == _socket) {
        ...
        int clientFileDescriptor =
            accept(_socket, reinterpret_cast<sockaddr *>(&clientAddress),
                   reinterpret_cast<socklen_t *>(&clientAddressLength));

        if (clientFileDescriptor == -1) break;

#ifdef DEBUG
        std::cerr << "Got connection from " << inet_ntoa(clientAddress.sin_addr)
                  << ':' << ntohs(clientAddress.sin_port) << std::endl;
#endif

        FD_SET(clientFileDescriptor, &masterSocketSet);
        newHighestFileDescriptor =
            std::max(highestFileDescriptor, clientFileDescriptor);

        auto clientSocket =
            std::make_shared<ClientSocket>(clientFileDescriptor, *this);

        if (_handleAccept)
          auto result =
              std::async(std::launch::async, _handleAccept, clientSocket);
      ...
      // Known client socket
        ...
        if (result <= 0) {
          ...
          if (itSocket != _clientSockets.end() && _handleRead)
            auto result =
                std::async(std::launch::async, _handleRead, *itSocket);
        }
      }
    }
    ...
    highestFileDescriptor =
        std::max(newHighestFileDescriptor, highestFileDescriptor);
